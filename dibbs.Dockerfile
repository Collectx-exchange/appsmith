...
RUN echo 'export const VERSION = "v1.8.4"' > ./app/rts/src/version.js
RUN cd ./app/rts && ./build.sh

RUN cd ./app/client && yarn install && REACT_APP_VERSION_ID=v1.8.4 REACT_APP_VERSION_RELEASE_DATE=$(date -u '+%Y-%m-%dT%H:%M:%SZ') \
  REACT_APP_CLIENT_LOG_LEVEL=ERROR EXTEND_ESLINT=true craco --max-old-space-size=4096 build --config craco.build.config.js

FROM 346284258841.dkr.ecr.us-east-1.amazonaws.com/appsmith-server:v1.8.4 as server
